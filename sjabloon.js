...
sliderBlur.addEventListener('input', function() {
    image.style.filter = `blur(${this.value}px) grayscale(${sliderZwartWit.value*10}%) contrast(${sliderContrast.value*10}%)`;
});
...
sliderZwartWit.addEventListener('input', function() {
    image.style.filter = `blur(${sliderBlur.value}px) grayscale(${this.value*10}%) contrast(${sliderContrast.value*10}%)`;
});
// if you change this slider than the contrast will wil increase or decrease
sliderContrast.addEventListener('input', function () {
    image.style.filter = `blur(${sliderBlur.value}px) grayscale(${sliderZwartWit.value*10}%) contrast(${this.value*10}%)`;
});
// if you change the value of the slider than the picture will turn
sliderDraaien.addEventListener('input', function () {
    image.style.filter = `blur(${sliderBlur.value}px) grayscale(${sliderZwartWit.value*10}%) contrast(${sliderContrast.value*10}%)`;
